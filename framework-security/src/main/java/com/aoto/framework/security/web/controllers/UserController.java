...
//    }
    ...
    {
    	//------------------------  密码输入错误次数达到上限就锁定    by tianjm  20181108  start -------------------------
    	String validateCodeMsg = request.getParameter("msg");//获取过滤器中传递过来的参数
    	String errorTimes = request.getParameter("errorTimes");//获取过滤器中传递过来的参数
    	if (validateCodeMsg != null && "nomatch".equals(validateCodeMsg)){ //
    		return new ModelAndView("shared/login").addObject("errorMessage", "验证码错误");
    	}
    	if (validateCodeMsg != null && "novaliate".equals(validateCodeMsg)){ //
    		return new ModelAndView("shared/login").addObject("errorMessage", "未能获取验证码");
    	}
    	if (validateCodeMsg != null && "pwdError".equals(validateCodeMsg)){ //
    		return new ModelAndView("shared/login").addObject("errorMessage", "用户名或密码错误");
    	}
    	if (validateCodeMsg != null && "userlock".equals(validateCodeMsg)){ //
    		return new ModelAndView("shared/login").addObject("errorMessage", "用户名已被锁定");
    	}
    	if(errorTimes!=null){
			if (validateCodeMsg != null && "errorTimes".equals(validateCodeMsg)) { //
				return new ModelAndView("shared/login").addObject("errorMessage", "密码输错" + errorTimes
							+ "次,该账户已锁定");
			}
			if (validateCodeMsg != null && "pwdWarn".equals(validateCodeMsg)) { //
				return new ModelAndView("shared/login").addObject("errorMessage", "密码输错"+errorTimes+"次将被锁定，已输错"+(Integer.parseInt(errorTimes)-1)+"次");
			}
    	}
    	//------------------------  密码输入错误次数达到上限就锁定    by tianjm  20181108  end -------------------------
    ...
    /*密码不合法需重置提示页面 20181106 tianjm*/
    @RequestMapping(value = "/login/pwd", method = RequestMethod.GET)
    public ModelAndView pwdChange(HttpServletRequest request){
    	String msg = "security_low";
    	return new ModelAndView("shared/password","msg",msg);
    }
    
    /*密码过期需重置提示页面 20181106 tianjm*/
    @RequestMapping(value = "/login/pwdOverdue", method = RequestMethod.GET)
    public ModelAndView pwdOverdue(HttpServletRequest request){
    	String pwdTimeLength = request.getParameter("pwdTimeLength");
    	return new ModelAndView("shared/password","pwdTimeLength",pwdTimeLength);
    }
