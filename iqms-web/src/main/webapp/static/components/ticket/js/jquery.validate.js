...
$.extend($.validator, {
	...
	prototype: {
		...
		showErrors: function( errors ) {
			...
			if ( this.settings.showErrors ) {
				//alert(this.errorMap+"  "+this.errorList);
				//errorList.
				/*this.errorList = [];
				for(var key in this.errorMap) {  
					alert(this.findByName(key)[0]);
				  	this.errorList.push({
						message: this.errorMap[key],
						element: this.findByName(key)[0]
					});
				}*/  
	...
	metadataRules: function(element) {
		...
		//alert($.toJSON($(element).metadata()[meta]));
		//alert($.toJSON($(element).metadata()));
