...
function loadDic()
{
    ...
    var queryDicName = $queryDicName.val();
    var queryDicType = $queryDicType.val();
    if(!/^[\u4e00-\u9fa5_a-zA-Z0-9]{0,32}$/i.test(queryDicName)){
    	alert('搜索的类型名称格式有误，字母数字下划线或者中文');
    	return;
    }
    if(! /^([a-zA-Z0-9]{0,32})$/i.test(queryDicType)){
    	alert('搜索的类型代码格式有误，输入值允许字母数字');
    	return;
    }
