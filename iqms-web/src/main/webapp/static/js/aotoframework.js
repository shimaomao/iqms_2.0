...
$.extend($.fn.validatebox.defaults.rules,
{
    ...
    {// 验证电话号码
        ...
        {
        	var res;
            return /^((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?$/i.test(value)||/^(13|14|15|17|18|19)\d{9}$/i.test(value);
    ...
    email :
    {// 验证邮箱  重写jquery.validate.js中的校验方式，避免非法字符的属于  by tianjm 20181109
    	validator : function(value)
    	{
//    		 var myreg = /^([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,3}$/;
    ...
    {// 验证用户名
        ...
        //message : '用户名不合法（字母开头，1-16字节，允许字母数字下划线）'
        message : '用户名不合法（由4-16位长度的字母数字下划线组成，不得使用root、admin、姓名简拼等 )'
    },
    menuUrl :
    {// 验证菜单URL
    	validator : function(value)
    	{
    			return /^[a-zA-Z0-9_/]{1,32}$/i.test(value);
    	},
    	message : '菜单URL不合法（1-32位，允许字母数字下划线斜杠）'
    },
    icon :
    {// 验证图标
    	validator : function(value)
    	{
    		return /^[a-zA-Z0-9_-]{1,16}$/i.test(value);
    	},
    	message : '图标不合法（1-16位，允许字母数字下划线横杠）'
    },
    urlPattern :
    {// 验证路径
    	validator : function(value)
    	{
    		return  /^[a-zA-Z0-9\\-_/\$\+*.]{1,32}$/i.test(value);
//    		return /^[a-zA-Z0-9_-/\.+*^\$]{1,32}$/i.test(value);
    	},
    	message : '路径不合法（1-32位，允许字母数字_/+\*.^$）'
    },
    code :
    {// 编码
        validator : function(value)
    ...
    {// 验证代码
    	...
    	message : '代码不合法（允许字母数字.和下划线，1-32字符）'
    },
    faxno :
    {// 验证传真
        validator : function(value)
