...
{	
	...
	jQuery(grid_selector).jqGrid({
		url:contextPath+'/report/businessstatistics?dt='+(new Date().getTime()),
		datatype:'json',
		mtype: 'GET',
		...
		colNames: ['机构编码', '机构名称', '业务名称', '客户类型','总受理量', '等待超时(人数)','等待未超时(人数)','办理超时(人数)','办理未超时(人数)'],
		colModel: [
			{name: 'orgCode',index: 'orgCode',width: 100,sortable: true}, 
			{name: 'orgName',index: 'orgName',width: 120,sortable: true}, 
			{name: 'businessName',index: 'businessName',width: 140,sortable: true}, 
			{name: 'custName',index: 'custName',width: 150,sortable: true}, 
			{name: 'dealCount',index: 'dealCount',width: 150,sortable: true}, 
			{name: 'waitCount',index: 'waitCount',width: 150,sortable: true},
			{name: 'notwaitCount',index: 'notwaitCount',width: 150,sortable: true}, 
			{name: 'dealwaitCount',index: 'dealwaitCount',width: 150,sortable: true},
			{name: 'notdealwaitCount',index: 'notdealwaitCount',width: 150,sortable: true}
		],
...
function  initFushioncharts(){
	...
	var chart1 = new FusionCharts(contextPath+"/static/lib/Charts/Column3D.swf", "chartdiv_01", "960","350");
...
/*function  getZheData(){
	...
	$.get(url,function(data,status){
	 	...
	 	strXML += "<categories> " +   
					 	  "<category label='等待超时' /> " +   
					      "<category label='等待未超时' /> " +   
					      "<category label='办理超时' /> " +   
					      "<category label='办理未超时' /> " +   
				  "</categories>  " ;
	 	
	   //拼接通过遍历不同的机构编码对应不同指标的情况
	 	for (var i = 0 ; i < length; i++){
		 		strXML +=  "<dataset seriesName='"+areaObject[i].orgName +":"+ areaObject[i].businessName +"' >" +
						 		"<set value='"+areaObject[i].waitCount+"' /> " +
						 		"<set value='"+areaObject[i].notwaitCount+"' /> " +
						 		"<set value='"+areaObject[i].dealwaitCount+"' /> " +
						 		"<set value='"+areaObject[i].notdealwaitCount+"' /> " +
				 		   "</dataset>";
	 	}
...
function btnExportAll(){
	window.location=contextPath+"/report/businessstatistics/excel?orgId="+$orgId.val()+
	"&busType="+$busType.find("option:selected").val()+
	"&startDate="+$startDate.val()+
	"&endDate="+$endDate.val()+
	"&page=-1&rows=-1&_="+new Date().getTime();
