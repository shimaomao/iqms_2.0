...
{	
	...
	jQuery(grid_selector).jqGrid({
		url:contextPath+'/report/cusanalyse?dt='+(new Date().getTime()),
		datatype:'json',
		mtype: 'GET',
		...
		colNames: ['机构编码', '机构名称', '客户类型', '总受理量', '平均等待时长（分钟）','最大等待时长（分钟）','最小等待时长（分钟）','平均受理时长（分钟）', '最大受理时长（分钟）','最小受理时长（分钟）'],
		colModel: [
			{name: 'orgCode',index: 'orgCode',width: 100,sortable: true}, 
			{name: 'orgName',index: 'orgName',width: 120,sortable: true}, 
			{name: 'custName',index: 'custName',width: 120,sortable: true}, 
			{name: 'dealCount',index: 'dealCount',width: 120,sortable: true}, 
	...
	$.ajax({
		...
		success: function(data) {
			...
			//alert("选择框的默认值："+ $custLevel.find("option:selected").val())
	...
	$.get(url,function(data,status){
	   ...
	   //拼接通过遍历不同的机构编码对应不同指标的情况
	 	for (var i = 0 ; i < length; i++){
		 		strXML +=  "<dataset seriesName='"+areaObject[i].orgName +":"+ areaObject[i].custName +"' >" +
...
function btnExportAll(){
	window.location=contextPath+"/report/cusanalyse/excel?orgId="+$orgId.val()+
	"&custLevel="+$custLevel.find("option:selected").val()+
	"&startDate="+$startDate.val()+
	"&endDate="+$endDate.val()+
	"&page=-1&rows=-1&_="+new Date().getTime();
