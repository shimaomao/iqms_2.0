...
{	
	...
	jQuery(grid_selector).jqGrid({
		url:contextPath+'/report/businesscount?dt='+(new Date().getTime()),
		datatype:'json',
		mtype: 'GET',
		...
		colNames: ['机构编码', '机构名称', '业务名称', '总受理量', '平均等待时长（分钟）','最大等待时长（分钟）','最小等待时长（分钟）','平均受理时长（分钟）', '最大受理时长（分钟）','最小受理时长（分钟）'],
		colModel: [
			{name: 'orgCode',index: 'orgCode',width: 100,sortable: true}, 
			{name: 'orgName',index: 'orgName',width: 140,sortable: true}, 
			{name: 'businessName',index: 'businessName',width: 130,sortable: true}, 
...
function btnExportAll(){
	window.location=contextPath+"/report/businesscount/excel?orgId="+$orgId.val()+
	"&busType="+$busType.find("option:selected").val()+
	"&startDate="+$startDate.val()+
	"&endDate="+$endDate.val()+
	"&page=-1&rows=-1&_="+new Date().getTime();
