		        ...
		        buttons:{
		            ...
		            ok: {
		                ...
		                action: function(){
		                	...
		                	var url = contextPath + "/basicconfig/orderconfig/copy?dt="+(new Date().getTime());
...
$(function()
{
	...
	jQuery(grid_selector).jqGrid({
		url:contextPath+'/basicconfig/orderconfig?dt='+(new Date().getTime()),
		datatype:'json',
		mtype: 'GET',
		...
		colNames: ['机构编码', '所属机构', '设备编号', '主机名称', '预约开始时间','预约结束时间','预约数量'],
		colModel: [
			{name: 'orgCode',index: 'orgCode',width: 180,sortable: true}, 
			{name: 'orgName',index: 'orgName',width: 180,sortable: false}, 
			{name: 'deviceNo',index: 'deviceNo',width: 180,sortable: false}, 
			{name: 'hostName',index: 'hostName',width: 180,sortable: false}, 
			{name: 'beginTime',index: 'beginTime',width: 180,sortable: false}, 
			{name: 'endTime',index: 'endTime',width: 180,sortable: false}, 
			{name: 'orderCount',index: 'orderCount',width: 180,sortable: false}
		],
...
//	$(".ui-jqgrid-bdiv").css("overflow-x","scroll");
	...
	var jqnav = jQuery(grid_selector).jqGrid('navGrid', pager_selector, { //navbar options
			//编辑按钮
		    edit: functionJson[191303],
			editicon: 'ace-icon fa fa-pencil blue',
			editfunc: editOrder,  //

			//添加按钮
			add: functionJson[191302],
			addicon: 'ace-icon fa fa-plus-circle purple',
			addfunc: newOrder,

			//删除按钮
			del: functionJson[191304],
