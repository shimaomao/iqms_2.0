...
$(function(){
	...
	//form绑定验证框架
	 $("#dialog-uploadinfo").find("form").validate({
...
function addtionalInfo(file)
{
	...
	//绑定对话框
	$newDialog = $("#dialog-uploadinfo").removeClass("hide").dialog({
		modal: true, //模式对话框
		...
		open:function(){ //加载数据
			//请空上一次的表单数据
			$("#dialog-uploadinfo").find("form").clearForm();
			//设置值
			var fileName = file.name;
			fileName = fileName.substring(0,fileName.lastIndexOf("."));
			$("#dialog-uploadinfo").find("#fileName").val(fileName);
		}
	});
}
