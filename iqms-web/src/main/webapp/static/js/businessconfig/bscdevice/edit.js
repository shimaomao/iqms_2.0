...
$(function(){
	...
	//绑定时间空监控
	$("#dialog-edit").find("form").find("input[name=installDate]").datetimepicker({
		  language: 'zh-CN',//显示中文
				...
				/**
				 ...
				 $.ajax({
						...
						success : function(result) {
							...
							var map = "";
							$("#screenResolution1").find("option").remove();
							var sel = $("#screenResolution1");
							for (var i=0; i<list.length; i++){
								map = list[i]; 
								if(rowData.screenResolution==map.key){
				...
				/**
				 ...
				 $.ajax({
						...
						success : function(result) {
							var list = result.rows;
							var map = "";
							$("#producer1").find("option").remove();
							var sel = $("#producer1");
							sel.append("<option id='"+map.key+"'value=''>请选择</option>");
							for (var i=0; i<list.length; i++){
								map = list[i]; 
								if(rowData.producer==map.key){
									sel.append("<option selected='selected' id='"+map.key+"'value='"+map.key+"'>" + map.value + "</option>");
								}else{
									sel.append("<option  id='"+map.key+"'value='"+map.key+"'>" + map.value + "</option>");
								}
...
function saveEdit(){
	 ...
	 //2.验证成功
	 var url = contextPath + "/businessconfig/bscdevice?_method=put&dt="+(new Date().getTime());
