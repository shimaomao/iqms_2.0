...
$(function (){
    ...
    function showModal(itemData, type)
    {
      ...
      //alert("showModal:"+$.toJSON(itemData));
      ...
      if(itemData.copyLevel == 'undefined'){
    	  $("#copyLevel").prop("checked",false);
      }else{
    	  $("#copyLevel").prop("checked",itemData.copyLevel);
      }
      $myModal.modal('show');
    }
    var dateType;
    ...
    function createBuz(){
    	$.each(result1.buzTypes, function (index1, item1){
    		$.each(result2.counterBuzs, function (index2, item2){
    			if(item1.busId == item2.busId){
    				item1.busCode = item2.businessCode;
    				return false; 
    			}
    		});
    		buzArr.push(item1);
    ...
    function modelSubmit()
    {
      ...
      var copyLevel = $("#copyLevel").prop("checked");
      var custLevelArr = [];
      ...
      $tr.data("itemData").copyLevel = copyLevel;
      var djData = $tr.data("itemData");
      djData.saveBuzStatus = true;
      if(copyLevel){
    	  if(dateType == 'pr'){
			  $("#prList tbody").empty().html("");
		  }else{
			  $("#trList tbody").empty().html("");
		  }
    	  $.each(buzArr, function (index1, item1){
    		  $.each(djData.custLevelArr, function (index2, item2){
    			  djData.custLevelArr[index2].businessCode = item1.busCode;
    			  djData.custLevelArr[index2].businessName = item1.businessName;
    			  djData.custLevelArr[index2].busId = item1.busId;
    		  });
    		  if(dateType == 'pr'){
    			  addTr("#prList tbody", djData, "pr");
    		  }else{
    			  addTr("#jrList tbody", djData, "jr");
    		  }
    	  });
      }
      $('#myModal').modal('hide');
