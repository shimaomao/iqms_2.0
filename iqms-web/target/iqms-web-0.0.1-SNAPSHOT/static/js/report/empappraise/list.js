...
{	
	...
	jQuery(grid_selector).jqGrid({
		url:contextPath+'/report/empappraise?dt='+(new Date().getTime()),
		datatype:'json',
		mtype: 'GET',
		...
		colNames: ['机构编码', '机构名称', '柜员工号', '柜员姓名', '满意','一般满意','不满意','未评价', '满意率','一般满意率','不满意率','未评价率'],
		colModel: [
			{name: 'orgCode',index: 'orgCode',width: 100,sortable: true}, 
			{name: 'orgName',index: 'orgName',width: 140,sortable: true}, 
			{name: 'workId',index: 'workId',width: 120,sortable: true}, 
			{name: 'workName',index: 'workName',width: 120,sortable: true}, 
			{name: 'appContent',index: 'appContent',width: 120,sortable: true}, 
			{name: 'appSoso',index: 'appSoso',width: 120,sortable: true},
			{name: 'appDiscontent',index: 'appDiscontent',width: 120,sortable: true}, 
			{name: 'appNonapp',index: 'appNonapp',width: 120,sortable: true},
			{name: 'rateContent',index: 'rateContent',width: 120,sortable: true,
				formatter:function(value,grid,rows,state){
					return value + "%" ;
				}
			}, 
			{name: 'rateSoso',index: 'rateSoso',width: 120,sortable: true,
				formatter:function(value,grid,rows,state){
					return value + "%" ;
				}
			},
			{name: 'rateDiscontent',index: 'rateDiscontent',width: 120,sortable: true,
				formatter:function(value,grid,rows,state){
					return value + "%" ;
				}
			}, 
			{name: 'rateNonapp',index: 'rateNonapp',width: 120,sortable: true,
				formatter:function(value,grid,rows,state){
					return value + "%" ;
				}
			}
...
function  initFushioncharts(){
	...
	var chart1 = new FusionCharts(contextPath+"/static/lib/Charts/StackedColumn3D.swf", "chartdiv_01", width,"560");
...
/*function  getZheData(){
	...
	$.get(url,function(data,status){
	 	...
	 	var strXML = "<chart caption='【"+" " +	 			
	 			...
	 			""+$endDate.val()+"】-柜员评价率情况' " +
	 			"yAxisName='单位/百分比' " +
		        "showNames='1' palette='5' numberSuffix='%'" +
	   ...
	   //拼接通过遍历不同的机构编码对应不同指标的情况
	 	for (var i = 0 ; i < length; i++){
		 		strXML +=  "<dataset seriesName='"+areaObject[i].orgName +"："+areaObject[i].workName +"' >" +
						 		"<set value='"+areaObject[i].rateContent +"' /> " +
						 		"<set value='"+areaObject[i].rateSoso +"' /> " +
						 		"<set value='"+areaObject[i].rateDiscontent +"' /> " +
						 		"<set value='"+areaObject[i].rateNonapp +"' /> " +
				 		   "</dataset>";
	 	}
...
function btnExportAll(){
	window.location=contextPath+"/report/empappraise/excel?orgId="+$orgId.val()+
	"&workName="+$workName.val()+
	"&startDate="+$startDate.val()+
	"&endDate="+$endDate.val()+
	"&page=-1&rows=-1&_="+new Date().getTime();
