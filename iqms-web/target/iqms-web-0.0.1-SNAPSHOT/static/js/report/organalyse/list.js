...
{	
	...
	jQuery(grid_selector).jqGrid({
		url:contextPath+'/report/organalyse?dt='+(new Date().getTime()),
		datatype:'json',
		mtype: 'GET',
		...
		colNames: ['机构编码', '机构名称', '总受理量', '平均等待时长（分钟）', '最大等待时长（分钟）','最小等待时长（分钟）','平均受理时长（分钟）','最大受理时长（分钟）','最小受理时长（分钟）'],
		colModel: [
			...
			{name: 'avgWait',index: 'avgWait',width: 150,sortable: true}, 
			{name: 'maxWait',index: 'maxWait',width: 150,sortable: true}, 
			{name: 'minWait',index: 'minWait',width: 150,sortable: true},
	...
	function updatePagerIcons(table) {
		...
		$('.ui-pg-table:not(.navtable) > tbody > tr > .ui-pg-button > .ui-icon').each(function() {
			var icon = $(this);
			//alert(icon.attr('class'))//ui-icon ui-icon-seek-first
			//alert(icon.attr('class').replace('ui-icon', '')) // ui-icon-seek-first
			var $class = $.trim(icon.attr('class').replace('ui-icon', ''));//$.trim()去掉字符串首尾空格。
...
/*function  getZheData(){
	...
	//请求的url
	var url = contextPath + "/report/organalyse/graphic?orgId="+$orgId.val()+
	...
	$.get(url,function(data,status){
	 	...
	 	strXML += "<categories> " +   
				      "<category label='平均等待时长' /> " +   
				      "<category label='最大等待时长' /> " +   
				      "<category label='最小等待时长' /> " +   
	   ...
	   //拼接通过遍历不同的机构编码对应不同指标的情况
	 	for (var i = 0 ; i < length; i++){
	 		strXML +=  "<dataset seriesName='"+areaObject[i].orgName +"' >" +
					 		"<set value='"+areaObject[i].avgWait+"' /> " +
					 		"<set value='"+areaObject[i].maxWait+"' /> " +
					 		"<set value='"+areaObject[i].minWait+"' /> " +
...
function btnExportAll(){
	window.location=contextPath+"/report/organalyse/excel?orgId="+$orgId.val()+
	"&startDate="+$startDate.val()+
	"&endDate="+$endDate.val()+
	"&page=-1&rows=-1&_="+new Date().getTime();
}
