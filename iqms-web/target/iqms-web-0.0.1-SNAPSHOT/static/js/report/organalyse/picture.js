			...
			var url = contextPath + "/report/organalyse/graphic?orgId="+$orgId.val()+
				...
				$.get(url,function(data){
					...
					for (var i = 0 ; i < length; i++){
						categories[i] = areaObject[i].orgName;
			   ...
			   //拼接通过遍历不同的机构编码对应不同指标的情况
			 	for (var i = 0 ; i < length; i++){
			 		strXML +=  "<dataset seriesName='"+areaObject[i].orgName +"' >" +
							 		"<set value='"+areaObject[i].dealCount+"' toolText='平均等待时间"+areaObject[i].avgWait+"分钟，平均办理时间"+areaObject[i].avgDeal+"分钟'/> " +
					 		   "</dataset>";
			 	}
			 	   strXML += "</chart>";
			 	...
			 	//遍历数据
			 	for (var i = 0 ; i < length; i++){
			 		strXML +=  "<set name='"+areaObject[i].orgName+"' value='"+areaObject[i].dealCount+"'" +
