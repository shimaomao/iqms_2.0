    ...
    function checkTicketModel(){
    	...
    	$.ajax({
    		...
    		data:jo,
    		success: function(data) {
    			var ticketForms = data.rows;  //窗口号+平日假日
    			if (ticketForms != null && ticketForms.length > 0){
    				...
    				for (var i = 0; i < ticketForms.length; i++){
    					...
    					//展示信息
    					if (ticketForm.ticketSytelFlag == 1) { //窗口配置成功
    ...
    function checkCustLevel(){
    	...
    	$.ajax({
    	  ...
    	  if(completeFlag){
    		  document.getElementById('checkPic').setAttribute("class","complete");
    	  }
    }
