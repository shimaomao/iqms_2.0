    ...
    $(function (){
	     ...
	     tktTmpId = ntktTmpId;  //号票模板ID
	     if(ntktTmpId!="" && ntktTmpId!=null){saveFlag = true}
	     tktTmpName = ntktTmpName;  //号票模板名称
	     orgId=norgId; //列表页面传递的机构号
	     deviceNo=ndeviceNo; //列表页面传递的设备编码
	     screenResolution= nscreenResolution; //设备分辨率，设备表中配置
...
//      $("label[name='tktName']").text(tktTmpName);
    ...
    var $orgDialog;
    var zTreeObj ;
    function selectTicketTree(orgId){
    	//1.定义ztree的配置
    	var setting = {
                view: {
    	...
    	//ajax请求加载数据
    	 $.ajax({
             url : contextPath + "/businessconfig/bscdevice/ztreeforticket?dt="+(new Date().getTime()),
             type : "get",
             dataType : "json",
             contentType : "application/json",
             data : {"orgId":orgId},
             success : function(datas)
             {
            	...
            	//初始化机构树
     			...
     			var title = '<i class="ace-icon fa fa-sitemap bigger-200 "></i><span class="ui-jqdialog-title" style="float: left;"> 设备选择</span>';
     			//打开对话框
     			$orgDialog = $("#dialog-org").removeClass("hide").dialog({
    				...
    				buttons: [{
    					...
    					click: function() {
    						...
    						if (selNodes != null && selNodes.length > 0){  //选择的节点大于0
    							var selNode = selNodes[0];
    							var tktTmpIdTemp = selNode.tktTmpId;  //号票模板ID
    							var orgIdTemp = selNode.pId;  //机构id
    							var deviceNoTmp = selNode.deviceNo;  //设备编号
    							//20181127 add by tianjm
    							var devName = selNode.devName;
    							$("#deviceName").val(devName);
    							
    							//查询次设备的号票模板信息，用于展示在到下面方式二中
    							$.ajax({
    						  		url: contextPath+"/businessconfig/ticketform/edit?ftype=select&_method=get", 
    						  		type : "get",
    						  		dataType: "json",
    						  		data:{"tktTmpId":tktTmpIdTemp,"orgId":orgIdTemp,"deviceNo":deviceNoTmp},
    						  		contentType: "application/json",
    						  		success: function(data) {
    						  			$tktTmpName.val(data.tktTmpName);  //设置号票模板名称
    						  			$def.prop("checked", true);  //设置默认模板
    						  			//设置号码列表的值
    						  			chose_set_ini("#tktFormatId", data.tktFormatId)
    						  			preview(data.tktTmpStyle);
    						  		},
    						  		error: function(msg) {
    						  			qmsVld.showError("出错了！");
    						  		}
...
//----------------------------------------------------左边取号元素  绑定事件 start--------------------------------------------      	    
    ...
    function saveConfig(){
      ...
      $.ajax({
    		url: contextPath+"/businessconfig/ticketform/addnavticket?dt="+(new Date().getTime()), 
    		type : "post",
    		dataType: "json",
    		...
    		success: function(data) {
    			 ...
    			 //data返回的是2部分数据
    			var retAttr = data.errorCode.split("#");
    			
    			 if (retAttr[0] == 0)
			      {
    				//设置号票模板值 
    				tktTmpId = retAttr[1];
    				if(tktTmpId!=""||tktTmpId!=null){
    					saveFlag = true;
    				}
