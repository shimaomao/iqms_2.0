...
//-----------------------------------------1、初始化加载   start---------------------------------------
	$(function(){
	     ...
	     styleId = mdStyleId;   //获取模板的id
	     if(styleId!="" && styleId!=null){saveFlag = true}
	     styleName = mdStyleName;  //获取模板的名称
	     orgId = mdOrgId;    //获取机构id
	     deviceNo = mdDeviceNo;  //获取设备编码
	     deviceName= mdDeviceName; //列表页面传递的设备编码
	     screenResolution = mdScreenResolution; // 获取设备的分辨率
	     orgName = params["orgName"];
		 devName = params["devName"];
	     var def = mdDef; //是否默认模板
	     //alert("设备分辨率："+screenResolution);
	     $def = $("#def");       //是否默认
	     //设置是否是默认模板(导航中为一直是默认模板)
...
//          $spn = $("<div class=\"site\" style=\"background-size:100% 100%;left:0px;top:0px;width:100px;height:50px;\" tabindex='" + tabIndex + "' id=\"item" + item.id + "\"><a href=\"javascript:void(0);\"><i style='display:inline-block;vertical-align: middle;'  class=\"fa fa-angle-up\"></i><li style='display:inline-block;vertical-align: middle;'>" + item.text + "<div style=\"font-size:5px;line-height:8px;\">specialCust</div><div style=\"font-size:5px;line-height:8px;\">等候人数：<span name=\"waitingNum\">0</span></div></li></a></div>");
    ...
    function saveAndPrintConfig(){
      ...
      $.ajax({
    	url: contextPath+"/businessconfig/takeconfig/saveConfig4nav?dt="+(new Date().getTime()), 
  		type : "post",
  		dataType: "json",
  		data:$.toJSON(args),
  		contentType: "application/json",
  		success: function(data) {
  			 ...
  			 if (returnCode == 0)
		      {
		        ...
		        //设置styleId = 
		        styleId = retAttr[1];  //20171213 by hongxz
		      }
    ...
    function nextStep(){
    	if(saveFlag){
    		//跳转到配置检测
         	window.location.href = encodeURI(contextPath + "/businessconfig/navconfig/check?dt="+(new Date().getTime())
         	+"&deviceNo="+deviceNo+"&orgId="+orgId+"&devName="+devName
         	+"&orgName="+orgName);
    	}else{
    		qmsVld.showError("请先配置并保存取号界面");
    	}
