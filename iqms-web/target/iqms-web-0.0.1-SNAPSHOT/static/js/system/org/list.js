...
function searchOrgs()
{
    ...
    var sData = {};
    var sOrgName = $sOrgName.val();
    var sOrgCode = $sOrgCode.val();
    if(!/^[\u4e00-\u9fa5_a-zA-Z0-9]{0,32}$/i.test(sOrgName)){
    	alert('机构名称格式有误，字母数字下划线或者中文');
    	return;
    }
    if(!/^([^\x00-\xff]|\w|\s){0,16}$/i.test(sOrgCode)){
    	alert('机构代码格式有误，不能包含特殊字符');
    	return;
    }
    if($sOrgName.val()) sData.text = sOrgName;
    if($sOrgCode.val()) sData.orgCode = sOrgCode;
    //alert($.toJSON(sData));
...
// remove org 原删除机构方法 现改为 删除机构级联删除 设备
/*function removeOrg()
{
...
{
    logBehavior("log.behavior.dialog.open", ["org.list", "remove"]);
    var messager = $.messager.confirm($.i18n.prop("org.remove"), "删除机构将级联删除机构下的设备及设备的相关信息，确定要删除吗？", function(r)
    {
        if (r)
        {
            var node = $org.tree("getSelected");
            
            var url = contextPath + "/system/orgs/extendDel/"+node.id +"?_method=delete";
            $.post(url, null, function(result)
            {
                if (result.errorCode != '0')
                {
                    $.messager.alert($.i18n.prop("messager.info"), "删除失败！");
                    logBehavior("log.behavior.dialog.close", ["org.remove"]);
                }
                else
                {
