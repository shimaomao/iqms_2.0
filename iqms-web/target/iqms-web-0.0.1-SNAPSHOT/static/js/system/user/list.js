...
function loadUsers()
{
    ...
    var name = $uname.val();
   if(!/^[a-zA-Z0-9_]{0,16}$/i.test(name)){
	   alert('搜索用户名不合法（0-16位，允许字母数字下划线）');
	   return;
   }
    param.username = name
    param.orgId = $oid.val();
            ...
            buttons : [
            {
                ...
                handler : function()
                {
                    ...
                    $.post(url, $frmNewUser.serialize(), function(result)
                    {
                        ...
                        {
                            $.messager.alert($.i18n.prop("messager.info"), $.i18n.prop("submit.success")+",已赋默认密码值，请及时修改", "info");
