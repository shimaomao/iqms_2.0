...
//				nspecial:true
	...
	jQuery(grid_selector).jqGrid({
		url:contextPath+'/basicconfig/menu?dt='+(new Date().getTime()),
		datatype:'json',
		mtype: 'GET',
		...
		colNames: [ '菜单名称', '菜单英文名'],
		colModel: [
			{name: 'menuName',index: 'menuName',width: 300,sortable: true}, 
			{name: 'menuEnName',index: 'menuEnName',width: 300,sortable: false}
		],
	...
	jQuery(grid_selector).jqGrid('navGrid', pager_selector, { //navbar options
			//编辑按钮
		    edit: functionJson[190603],
			editicon: 'ace-icon fa fa-pencil blue',
			editfunc: editMenu,  //

			//添加按钮
			add: functionJson[190602],
			addicon: 'ace-icon fa fa-plus-circle purple',
			addfunc: newMenu,

			//删除按钮
			del: functionJson[190604],
...
function loadDatas()
{
	//获取参数对象
	var reqParam = jQuery(grid_selector).jqGrid("getGridParam", "postData");
	var menuName = jQuery("#searchFrom").find("input[name=menuName]").val();
	reqParam.menuName = menuName;
