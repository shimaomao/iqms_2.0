...
$(function(){
	...
	//form绑定验证框架
	  ...
	  */
	 $.ajax({
			url : contextPath + "/basicconfig/sysdic/sexType?_method=get&dt="
					+ (new Date().getTime()),
			type : "GET",
	  ...
	  */
	 $.ajax({
			url : contextPath + "/basicconfig/sysdic/tellerStatus?_method=get&dt="
					+ (new Date().getTime()),
			type : "GET",
			contentType : "application/json; charset=utf-8",
			dataType : "json",
...
function save(){
	...
	//1.验证失败
	 ...
	 var url = contextPath + "/basicconfig/teller?dt="+(new Date().getTime());
