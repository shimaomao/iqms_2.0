        ...
        <div class="col-md-8 vertical-center center">            
            <h1>
                ...
                <span class="white"><fmt:message key="login.application" /></span>
                 -->
                <span class="white">智能分流与营销系统</span>
            </h1>
            <h4 class="blue">&copy; <fmt:message key="login.remark" /></h4>            
        </div>
        <div class="col-md-4 vertical-center" style="op">
            <form id="loginForm" style="padding-bottom: 20px;" class="form-horizontal" action="${contextPath}/j_spring_security_check" method="post">
                <span class="heading" style="padding:10px;"><fmt:message key="login.title" /></span>                
                ...
                <div class= "form-group">
			   		<input  id="veryCode" name="j_veryCode" class="form-control" placeholder="验证码<fmt:message key="colon" />"tabindex="3"/>         			
			   </div>
			   
			   <div class= "form-group">			  		
					<img src="" id="imgObj" /> 
					<a href="javascript:void(0);" id="a1" onclick="changeImg()">换一张</a>
					
			   </div>
                <div class="form-group">
                    <span class="text" id="spnErr">${errorMessage}</span>
                    <button type="button" onclick="javascript:sub_login();" class="btn btn-default"><fmt:message key="login" /></button>
                </div>
            </form>
        </div>
    </div>
